...
$eid=$_SESSION['emp_id'];
$sql="SELECT * FROM jobs  WHERE e_id='$eid'";
$data=mysqli_query($connection,$sql);
    ?>
<?php
if (isset($_GET['ajid'])) {
    $aj_id = $_GET['ajid'];
    $mydb="DELETE FROM `applied_jobs` WHERE aj_id='$aj_id'";
    $result = mysqli_query($connection,$mydb);
    if($result)
    {
            echo "<script>alert('Sucessfully Rejected!!.')</script>";
        }
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="banner-text text-center">
                        <h1>Employer Candidates</h1>
                        <ul class="breadcumb list-inline">
                            <li class="list-inline-item"><a href="index.php">Home</a></li>
                            <li class="list-inline-item">Employer Candidates</li>
                        </ul>
                    </div>
                </div>
            </div>
...
{
$jobid=$res['job_id'];
$sql="SELECT job_seekers.*,resume.*,applied_jobs.aj_id FROM ((`applied_jobs` inner join `job_seekers` on applied_jobs.s_id=job_seekers.s_id) inner join resume on resume.s_id=applied_jobs.s_id) WHERE applied_jobs.job_id='$jobid'";
...
foreach ($result1 as $row) {
    ...
    <main class="main-content-area">
      ...
      <a class="btn btn-success" href="files/<?php echo $row['file']?>">
                        Download Resume</a>
                        <a class="btn btn-danger" href="emp_candidates.php?ajid=<?php  echo $row['aj_id'] ?>">
                        Reject Resume</a>
                            </div>
                        </div>
